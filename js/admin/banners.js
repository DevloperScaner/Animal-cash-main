...
window.renderAdminBanners = async function(){
  ...
  for(const d of snap.docs){
    const x=d.data(); const tr=document.createElement('tr');
    tr.innerHTML=`<td>${x.title||'-'}<div class='small'>${x.subtitle||''}</div></td><td><code>${x.img||''}</code></td><td>${x.active? 'Ya':'Tidak'}</td><td><button class='btn' data-toggle='${d.id}'>Toggle</button> <button class='btn red' data-del='${d.id}'>Hapus</button></td>`; tb.appendChild(tr);
  }
  el.querySelector('#fAdd')?.addEventListener('submit', async (e)=>{
    e.preventDefault();
    const title = el.querySelector('#title').value.trim();
    const subtitle = el.querySelector('#subtitle').value.trim();
    const img = el.querySelector('#img').value.trim();
    const active = el.querySelector('#active').checked;
    await addDoc(collection(db,'banners'), { title, subtitle, img, active, createdAt: serverTimestamp() });
    window.render('banners');
  });
  tb.addEventListener('click', async (e)=>{
    const t=e.target.closest('button[data-toggle]'); const d=e.target.closest('button[data-del]');
    if(t){ const id=t.dataset.toggle; const ref=doc(db,'banners',id); const s=await (await import('https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js')).getDoc(ref); if(s.exists()){ await updateDoc(ref, { active: !s.data().active }); } window.render('banners'); }
    if(d){ await deleteDoc(doc(db,'banners',d.dataset.del)); window.render('banners'); }
  });
};
