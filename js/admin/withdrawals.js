...
window.renderAdminWithdrawals = async function(){
  ...
  tb.addEventListener('click', async (e)=>{ const a=e.target.closest('button[data-approve]'); const r=e.target.closest('button[data-reject]'); if(a){ await updateDoc(doc(db,'withdrawals',a.dataset.approve), { status:'approved', approvedAt:new Date() }); window.render('withdrawals'); } if(r){ await updateDoc(doc(db,'withdrawals',r.dataset.reject), { status:'rejected', rejectedAt:new Date() }); window.render('withdrawals'); } });
};
